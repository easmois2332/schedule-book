...
const buttonScreenShot = async () => {
  const el = document.querySelector('#content-view-area');
  const canvasElement = await html2canvas(el, {}).catch(e => {
    console.error(e);
  });
  canvasElement.toBlob(async blob => {
    ...
    object.href = blobUrl;
    object.download = `ScheduleBook_${(new Date()).getTime()}.png`;
  ...
  <header id="header-area" class="header-area" v-bind:class="settingColor">
    ...
    <div class="header-operation-area">
      ...
      <div class="header-operation screen-shot">
        <button class="header-operation-button" v-bind:disabled="currentComponent !== ScheduleView" @click="buttonScreenShot">
          <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor">
